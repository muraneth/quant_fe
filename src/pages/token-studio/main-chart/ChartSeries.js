...
export const generateXSeries = (theme,klineData,chartData) => {
    let defaultXSeries = [];
...
export const generateYSeries = (theme,klineData,chartData) => {
    let minPrice = 0;
    if (klineData?.length > 0) {
    ...
    if (chartData?.length > 0) {
        ...
        if (pbvIndicatorData){
            ...
            defaultYSeries[0].max = numberFormatter(maxPrice);
            defaultYSeries.push({
                type: 'category',
